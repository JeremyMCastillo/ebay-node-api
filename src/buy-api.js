...
const getItem = function (itemId) {
  ...
  return makeRequest(
    ...
    `/buy/browse/v1/item/${id}?fieldgroups=PRODUCT`,
    "GET",
    this.options.body,
    auth
  ).then((result) => {
    console.log("TCL: getItem -> result", result);
...
const searchItems = function (searchConfig) {
  ...
  queryParam += searchConfig.categoryId ? `&category_ids=${searchConfig.categoryId}` : "";
  ...
  if (searchConfig.filter != undefined) queryParam = `${queryParam}&filter=${encodeURIComponent(
